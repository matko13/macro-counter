...
| nazwę i wartości produktu | 688 gotowych produktów z polskiej kuchni w bazie |
...
| cały posiłek po kawałku | **zestawy** — raz zapisany posiłek dodajesz jednym tapnięciem, a ołówek pozwala go poprawić: nazwa, gramatura każdego składnika, wyrzucenie i dorzucenie produktu. Przy dodawaniu do dnia wybierasz, **ile z tego zjadłeś** — sam zestaw zostaje pełną definicją posiłku |
...
- **Pinsa Margherita (Lidl)** — jedyny wpis markowy, który jest **szacunkiem
  mimo istnienia etykiety**: producent jej nie publikuje, a dane społecznościowe
  rozjeżdżają się o 30% (199 / 236 / 262 kcal na 100 g). Rozstrzygnięte
  składem: spód, sos i mozzarella dają 204–208 kcal/100 g niezależnie od
  przyjętych proporcji, co wyklucza 262. Test pilnuje przedziału 190–215,
  więc wpisanie tam 262 zapali się na czerwono
...
Wbudowane 688 pozycji to produkty ogólne. Konkretny jogurt konkretnej marki
...
- Testy: **721 przypadków** przez Playwright (headless Chromium, część na
  ...
  | dotyk i geometria | 24 | żadna reguła `:hover` poza `@media (hover:hover)` (na iOS pierwsze tapnięcie na takim elemencie tylko „najeżdża", a klika dopiero drugie), pojedyncze tapnięcie zatwierdza posiłek i dodaje produkt, **każdy element dotykowy ma ≥44 px** na wszystkich ekranach, oraz szerokość toastu: pełna szerokość treści zamiast połowy ekranu, równe marginesy, najwyżej dwie linijki tekstu i nieściśnięty „Cofnij". Do tego **rozmiary wszystkich ikon na każdym ekranie** — funkcja rysująca ikony nie podaje wymiarów, a bezwymiarowy SVG w gridzie rozciąga się na całe pole, a we fleksie zapada do zera i po prostu znika. Oraz **arkusz a klawiatura iOS**: klawiatura nie zmniejsza okna, tylko je nakrywa, więc krótki arkusz z polami chował się pod nią w całości — test udaje ten stan (`visualViewport` kurczy się, `innerHeight` zostaje) i sprawdza, że arkusz podnosi się nad klawiaturę, że „Zapisz” jest w zasięgu i że po zamknięciu geometria wraca do domyślnej |
  ...
  | zestawy: edycja i dodawanie | 39 | nazwa, gramatura składnika z przeliczeniem kcal, wyrzucenie i dorzucenie produktu przez szukanie, „Anuluj" naprawdę nic nie zmieniające (praca na kopii), odmowa zapisu pustego zestawu i bez nazwy, poprawiona wersja wpadająca do dnia, oraz składnik po usuniętym produkcie skalowany proporcjonalnie zamiast blokować edycję. Do tego **ile z zestawu zjadłeś**: ułamek wybierany przy dodawaniu (nie zapisywany w zestawie), skalowanie gramatur i kcal, mililitry pozostające mililitrami, nietknięta definicja po dodaniu połowy, powrót do całości przy następnym dodaniu i „Anuluj” nic nie dopisujący |
